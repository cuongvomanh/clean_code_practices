public class TestableHtmlRefactorUncleBobMidRefactor {


    public static String renderPageWithsetupsAndTeardowns(
            PageData pageData, boolean isSuite
    ) throws Exception {
        bollean isTestPage = pageData.hasAttribute("Test");
        if (isTestPage) {
            WikiPage testPage = pageData.getWikiPage();
            StringBuffer newPageContent = new StringBuffer();
            includeSetupPages(testPage, newPageContenet, isSuite);
            newPageContent.append(pageData.getContent());
            includeTeardownPages(testPage, newPageContent, isSuite);
            pageData.setContenet(newPageContent.toString());
        }
        return pageData.getHtml();
    }

    private static void includeSetupPages(WikiPage wikiPage, StringBuffer newPageContent, boolean isSuite){
        if (isSuite) {
            ...
            if (suiteSetup != null) {
                ...
                newPageContent.append(INCLUDE_SETUP)
                        .append(pagePathName)
                        .append("\n");
            }
        }
    ...
    private static void includeTeardownPages(WikiPage wikiPage, StringBuffer newPageContent, boolean isSuite){
        WikiPage setup = PageCrawlerImp.getInheritedPage("Setup", wikiPage);
        if (setup != null) {
            WikiPagePath setupPath = wikiPage.getPageCrawler().getFullPath(setup);
            String setupPathName = PathParser.render(setupPath);
            newPageContent.append("!includ =setup .")
